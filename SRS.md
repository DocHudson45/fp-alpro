...
**Request body:**
```typescript
{ targetTool: "stitch" | "framer" | "v0" | "bolt" | "webflow" | "lovable" | "replit" | "claude" }
```
...
**Purpose:** Generate moodboard/concept image on-demand using Nano Banana.
...
1. Fetch project + analysis.
2. If analysis missing, return 400.
3. Build image prompt from `analysis.visualDirection` (see §6.5).
4. Call Gemini `gemini-2.5-flash-image` model.
5. Receive image binary, upload to Supabase Storage at `generated-images/{projectId}/{cuid}.png`.
6. Save `GeneratedImage` record.
